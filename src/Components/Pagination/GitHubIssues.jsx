...
function GitHubIssues() {
  ...
  return (
    <div className="py-8 sm:px-5">
      ...
      <div
        className="border-y rounded-none sm:border ParentDiv bg-[#161b22] text-slate-100 w-[100%] sm:w-[80%] m-auto sm:rounded-t-md"
        style={{
          display: 'flex',
      ...
      >
        <div
          className="flex items-center firstDiv gap-x-3"
          style={{ margin: '2% 2%' }}
        >
      ...
      {/* data Map here  */}
      <div className=" mapContaner w-[100%] sm:w-[80%]">
        {issues.length > 0 &&
          issues.map((el,ind) => {
            ind = ind+1;
            return (
              <div
                ...
                className={` border-y px-2 sm:border cursor-pointer border-slate-100 hover:bg-[#161b22] group flex  sm:px-6 py-1 sm:py-5 items-center gap-x-4 ${ind%30===0 ? "sm:rounded-b-md" :"rounded-none" }`}
              >
               ...
               <p  className='group-hover:text-[#1f6feb] break-words text-sm sm:text-lg font-semibold'>{el.title}</p>
               <div className='flex text-[10px]  sm:text-sm break-words  gap-x-1   sm:flex-row' >
                 #{el.number} <p >opened</p>{' '}
                 <TimeAgo  time={el.created_at} />{' '}
                 <p className='' > by</p>{' '}
                 <span  className="break-words userDetails">{el.user.login}</span>
                 {el.comments > 0 && (
                   <span
